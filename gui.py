...
class CircularTimerWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.hour_input = QLineEdit("00"); self.hour_input.setObjectName("time_input"); self.hour_input.setValidator(QIntValidator(0, 99))
        self.minute_input = QLineEdit("25"); self.minute_input.setObjectName("time_input"); self.minute_input.setValidator(QIntValidator(0, 59))
        self.second_input = QLineEdit("00"); self.second_input.setObjectName("time_input"); self.second_input.setValidator(QIntValidator(0, 59))
        colon1 = QLabel(":"); colon1.setObjectName("time_colon"); colon2 = QLabel(":"); colon2.setObjectName("time_colon")
        input_layout = QHBoxLayout(); input_layout.setAlignment(Qt.AlignmentFlag.AlignCenter); input_layout.addStretch(); input_layout.addWidget(self.hour_input); input_layout.addWidget(colon1); input_layout.addWidget(self.minute_input); input_layout.addWidget(colon2); input_layout.addWidget(self.second_input); input_layout.addStretch()
        main_layout = QVBoxLayout(self); main_layout.addStretch(); main_layout.addLayout(input_layout); main_layout.addStretch()
        self.input_widgets = [self.hour_input, self.minute_input, self.second_input, colon1, colon2]
    def set_time(self, total_seconds, current_seconds_float): self.total_seconds = total_seconds; self.current_seconds_float = current_seconds_float; self.update()
    def set_inputs_visible(self, visible):
        for widget in self.input_widgets: widget.setVisible(visible)
    def paintEvent(self, event):
        painter = QPainter(self); painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        rect = self.rect(); side = min(rect.width(), rect.height()); margin = 15
        drawing_rect = QRectF((rect.width() - side) / 2 + margin, (rect.height() - side) / 2 + margin, side - 2 * margin, side - 2 * margin)
        bg_pen = QPen(QColor("#3c3c3c"), 12, Qt.PenStyle.SolidLine); painter.setPen(bg_pen); painter.drawEllipse(drawing_rect)
        if self.total_seconds > 0:
            progress_pen = QPen(QColor("#0078d7"), 14, Qt.PenStyle.SolidLine); painter.setPen(progress_pen)
            ...
            arc_angle = progress_ratio * 360; start_angle = 90 * 16; span_angle = -int(arc_angle * 16)
        ...
        if not self.hour_input.isVisible():
            ...
            hours = current_seconds_int // 3600; minutes = (current_seconds_int % 3600) // 60; seconds = current_seconds_int % 60
            if self.total_seconds >= 3600: time_text = f"{hours:02}:{minutes:02}:{seconds:02}"; font_size = 30
            else: time_text = f"{minutes:02}:{seconds:02}"; font_size = 40
            font = QFont("Segoe UI", font_size); font.setBold(True)
            painter.setFont(font); painter.setPen(QColor("#f0f0f0")); painter.drawText(drawing_rect, Qt.AlignmentFlag.AlignCenter, time_text)

class Ui_BlockerApp(object):
    def setupUi(self, main_window):
        main_window.setWindowTitle('PyQt System Blocker')
        screen = QScreen.availableGeometry(QApplication.primaryScreen())
        width = int(screen.width() * 0.4); height = int(screen.height() * 0.6)
        main_window.setGeometry(screen.x(), screen.y(), width, height)
        self.main_layout = QVBoxLayout(main_window); self.main_layout.setContentsMargins(1, 1, 1, 1); self.main_layout.setSpacing(0)
        self.title_bar = QWidget(); self.title_bar.setObjectName("title_bar"); self.title_bar.setFixedHeight(35)
        title_bar_layout = QHBoxLayout(self.title_bar); title_bar_layout.setContentsMargins(10, 0, 0, 0)
        self.title_label = QLabel('PyQt System Blocker'); self.title_label.setObjectName("title_label")
        self.minimize_button = QPushButton(); self.minimize_button.setObjectName("minimize_button")
        self.maximize_button = QPushButton(); self.maximize_button.setObjectName("maximize_button")
        self.close_button = QPushButton(); self.close_button.setObjectName("close_button")
        ...
        self.reset_button = QPushButton("Reset"); self.reset_button.setObjectName("reset_button")
        timer_button_layout.addStretch(); timer_button_layout.addWidget(self.start_button); timer_button_layout.addWidget(self.reset_button); timer_button_layout.addStretch()
        ...
        charts_tab = QWidget(); charts_tab.setLayout(QVBoxLayout()); charts_tab.layout().addWidget(QLabel("Página de Gráficos"))
